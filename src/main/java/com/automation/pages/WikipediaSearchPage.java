...
public class WikipediaSearchPage {
    ...
    public void verifySearchBoxValue(String value) {
        ...
        page.locator("#searchInput").hasValue(value);
    }
    
    public void verifySearchResultsText(String text) {
        System.out.println("📍 The search results page shows '" + text + "'");
        page.locator("//h1[contains(normalize-space(.),'Search results for')]").waitFor(new Locator.WaitForOptions().setTimeout(2000));
        page.locator("//h1[contains(normalize-space(.),'Search results for')]").textContains(text);
    }
    
    public void verifyMultipleSearchResults() {
        System.out.println("📍 Multiple search results are listed");
        page.locator(".mw-search-results").waitFor(new Locator.WaitForOptions().setTimeout(2000));
        page.locator(".mw-search-result-main-content").isVisible();
        page.locator(".mw-search-result-main-content").count() > 1;
    }
    ...
    public void verifyArticleContentVisible() {
        ...
        page.locator("#mw-content-text").waitFor(new Locator.WaitForOptions().setTimeout(2000));
