...
public class WikipediaSearchPage {
    ...
    public void verifyMultipleSearchResults() {
        ...
        page.locator(".mw-search-results li").waitFor(new Locator.WaitForOptions().setTimeout(2000));
        int count = page.locator(".mw-search-results li").count();
        if (count > 1) {
            System.out.println("Multiple search results found: " + count);
        } else {
            System.out.println("Only one search result found");
        }
    }
